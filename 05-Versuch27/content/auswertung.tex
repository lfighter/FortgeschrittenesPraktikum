...
Auf Basis von Formel \eqref{eq:disGebiet} und Formel \eqref{eq:deltaLambda} mit den Konstanten$\lambda = \SI{643.8}{\nano\meter}$ und $n = 1.4567$ ergeben sich die einzelnen $\sigma\lambda_\text{norm,\sigma}$ in Tabelle \ref{tab:deltsigmas}. Ein mittleres $\sigma\lambda$ wird über eine konstante Ausgleichsrechnung der Form $c$ zu $\SI{11.7(1)}{\pico\meter}$ bestimmt. Der Landefaktor berechnet sich mithilfe von Formel \eqref{eq:exgj} und bei einer angelegten Stromstärke von $\SI{9.5}{\ampere}$ %konstanten angeben
